...
extern "C" __global__ void flashattn_gqa_decode_split_kernel(half_t* __restrict__ K, __grid_constant__ const CUtensorMap Output_partial_desc, __grid_constant__ const CUtensorMap Q_desc, half_t* __restrict__ V, int* __restrict__ cache_seqlens, float* __restrict__ glse, int* __restrict__ head_mapping, int* __restrict__ topk_indices, int batch, int max_cache_seqlen, int topk, float topk_percent);
...
extern "C" __global__ void __launch_bounds__(256, 1) flashattn_gqa_decode_split_kernel(half_t* __restrict__ K, __grid_constant__ const CUtensorMap Output_partial_desc, __grid_constant__ const CUtensorMap Q_desc, half_t* __restrict__ V, int* __restrict__ cache_seqlens, float* __restrict__ glse, int* __restrict__ head_mapping, int* __restrict__ topk_indices, int batch, int max_cache_seqlen, int topk, float topk_percent) {
  ...
  __syncthreads();
  int topk_num = (((int)((topk_percent * 0x1.47ae147ae147bp-7f/*1.000000e-02*/) * ((float)cache_seqlens[((int)blockIdx.x)]))) + 127);
  int condval;
  if ((((int)blockIdx.z) < ((topk_num & 1023) >> 7))) {
    condval = 1;
  } else {
    condval = 0;
  }
  if (128 <= ((int)threadIdx.x)) {
    ...
    int64_t kv_offset = ((((int64_t)((int)blockIdx.x)) * ((int64_t)max_cache_seqlen)) * (int64_t)1024) + (((int64_t)((int)blockIdx.y)) * (int64_t)128);
    int64_t topk_offset = (((min(((int64_t)((int)blockIdx.z)), ((topk_num & (int64_t)1023) >> (int64_t)7)) * (int64_t)128) + (((topk_num >> (int64_t)10) * ((int64_t)((int)blockIdx.z))) * (int64_t)128))) + (((((int64_t)((int)blockIdx.x)) * (int64_t)8) + ((int64_t)head_mapping[((int64_t)((int)blockIdx.y))])) * ((int64_t)topk));
    for (int k = 0; k < ((topk_num >> 10) + condval); ++k) {
    ...
    tl::fence_proxy_async();
    mbarrier[8].wait(0);
    for (int k_1 = 0; k_1 < ((topk_num >> 10) + condval); ++k_1) {
      #pragma unroll
      ...
      for (int i_8 = 0; i_8 < 2; ++i_8) {
        ...
        for (int rv = 0; rv < 32; ++rv) {
          scores_max[i_8] = max(scores_max[i_8], acc_s[((((rv & 15) * 4) + (i_8 * 2)) + (rv >> 4))]);
        }
        scores_max[i_8] = tl::AllReduce<tl::MaxOp, 4, 1, 0, 128>::run_hopper(scores_max[i_8]);
      }
      #pragma unroll
      ...
      for (int i_11 = 0; i_11 < 2; ++i_11) {
        ...
        for (int rv_1 = 0; rv_1 < 32; ++rv_1) {
          scores_sum[i_11] = (scores_sum[i_11] + acc_s[((((rv_1 & 15) * 4) + (i_11 * 2)) + (rv_1 >> 4))]);
        }
        scores_sum[i_11] = tl::AllReduce<tl::SumOp, 4, 1, 0, 128>::run_hopper(scores_sum[i_11]);
      }
      #pragma unroll
      ...
      for (int i_13 = 0; i_13 < 32; ++i_13) {
        uint1 __1;
        float2 v_ = *(float2*)(acc_s + (i_13 * 2));
        ((half2*)(&(__1.x)))->x = (half_t)(v_.x);
        ((half2*)(&(__1.x)))->y = (half_t)(v_.y);
        *(uint1*)(acc_s_cast + (i_13 * 2)) = __1;
      }
      #pragma unroll
    ...
    for (int i_16 = 0; i_16 < 2; ++i_16) {
      logsum[i_16] = (__log2f(logsum[i_16]) + (scores_max[i_16] * 0x1.0527dbd5cafffp-3f/*1.275174e-01*/));
    }
    #pragma unroll
    for (int i_17 = 0; i_17 < 2; ++i_17) {
      if (((((((int)threadIdx.x) >> 5) * 4) + (i_17 * 2)) + ((((int)threadIdx.x) & 31) >> 4)) < 1) {
        glse[((((((((int64_t)((int)blockIdx.x)) * (int64_t)256) + ((((int64_t)((int)threadIdx.x)) >> (int64_t)5) * (int64_t)128)) + (((int64_t)i_17) * (int64_t)64)) + (((int64_t)((int)blockIdx.y)) * (int64_t)32)) + (((((int64_t)((int)threadIdx.x)) & (int64_t)31) >> (int64_t)2) * (int64_t)8)) + ((int64_t)((int)blockIdx.z)))] = logsum[i_17];
      }
    }
    tl::__sync_thread_partial<3, 128>();
    #pragma unroll
    for (int i_18 = 0; i_18 < 32; ++i_18) {
      if ((((((((int)threadIdx.x) >> 5) * 4) + ((i_18 & 1) * 2)) + ((((int)threadIdx.x) & 31) >> 4)) < 1) && (((((((int)threadIdx.x) >> 5) * 4) + ((i_18 & 1) * 2)) + ((((int)threadIdx.x) & 31) >> 4)) < 1)) {
        *(float2*)(((float*)buf_dyn_shmem) + (((((((((int)threadIdx.x) >> 5) * 2048) + ((i_18 & 1) * 1024)) + (((((int)threadIdx.x) & 31) >> 2) * 128)) + ((i_18 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + 32768)) = *(float2*)(acc_o + (i_18 * 2));
      }
    }
    tl::fence_proxy_async();
    tl::__sync_thread_partial<3, 128>();
...
extern "C" int call(half_t* __restrict__ Q, half_t* __restrict__ K, half_t* __restrict__ V, int* __restrict__ cache_seqlens, int* __restrict__ topk_indices, int* __restrict__ head_mapping, float topk_percent, float* __restrict__ glse, float* __restrict__ Output_partial, half_t* __restrict__ Output, int batch, int max_cache_seqlen, int topk, cudaStream_t stream=cudaStreamDefault) {
	...
	flashattn_gqa_decode_split_kernel<<<dim3(batch, 8, 8), dim3(256, 1, 1), 147456, stream>>>(K, Output_partial_desc, Q_desc, V, cache_seqlens, glse, head_mapping, topk_indices, batch, max_cache_seqlen, topk, topk_percent);
