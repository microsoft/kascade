...
class NoRemapKascadeStrategy(PostSoftmaxPooledPrefillTopkStrategy): # Does not support stats collection since never used for stats collection
    def __init__(self, recompute_layers: List[int], k=1, tile_size=1, rolling_prefill=False, block_size=12288):
        super().__init__(name="no_remap_kascade", k=k, tile_size=tile_size, rolling_prefill=rolling_prefill, block_size=block_size)
        ...
        **kwargs,
        ...
        if attention_mask is not None:
            ...
            if not (module.layer_idx in self.recompute_layers
                ...
                indices = self._topk_to_reuse[:, :, start:end, :].to(torch.int64)
